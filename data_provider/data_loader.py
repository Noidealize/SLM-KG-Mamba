...
def _load_data_stamp(root_path, data_path):
    data_name = data_path.split('.')[0]
    stamp_path = os.path.join(root_path, f'{data_name}.pt')
    if not os.path.exists(stamp_path):
        raise FileNotFoundError(
            f"Missing preprocessed timestamp embeddings: {stamp_path}. "
            f"Run preprocess.py/preprocess_gpt.py/preprocess_opt.py for {data_name} first."
        )
    try:
        return torch.load(stamp_path, map_location='cpu', weights_only=True)
    except TypeError:
        return torch.load(stamp_path, map_location='cpu')
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        self.data_stamp = _load_data_stamp(self.root_path, self.data_path)
