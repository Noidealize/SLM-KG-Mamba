...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.experts = nn.ModuleList([
            nn.Linear(self.hidden_dim_of_gpt2, self.hidden_dim_of_gpt2, bias=False)
            for _ in range(self.num_experts)
        ])
        self.gate = nn.Linear(self.hidden_dim_of_gpt2, self.num_experts)
        ...
        if self.fusion_gate is not None:
            if x_mark_enc.shape[-1] != self.hidden_dim_of_gpt2:
                raise ValueError(
                    ...
                    f"{self.hidden_dim_of_gpt2}; rerun preprocess_gpt.py with the same --llm_ckp_dir."
                )
